...
// 商品コードマッピングよりCROS regular_flgを決定する
function getProductRegularFlg(productId, category1, category2) {
  for (var i = 0; i < PRODUCT_CATEGORY_MAPPING.length; i++) {
    if (PRODUCT_CATEGORY_MAPPING[i].product_id == productId) {
      for (var j = 0; j < PRODUCT_CATEGORY_MAPPING[i].product_code_mapping.length; j++) {
        if (PRODUCT_CATEGORY_MAPPING[i].product_code_mapping[j].category1 == category1 && PRODUCT_CATEGORY_MAPPING[i].product_code_mapping[j].category2 == category2) {
          return PRODUCT_CATEGORY_MAPPING[i].product_code_mapping[j].isRegular ? PRODUCT_CATEGORY_MAPPING[i].product_code_mapping[j].isRegular : 0;
        }
      }
    }
  }
...
function getSelectedProductJson() {
    ...
    for (var j = 0; j < CATEGORY_1_LIST.list.length; j++) {
      for (var k = 0; k < CATEGORY_2_LIST.list.length; k++) {
        ...
        if (quantity > 0) {
          ...
          var productRegular = getProductRegularFlg(productId, CATEGORY_1_LIST.list[j].value, CATEGORY_2_LIST.list[k].value);
          var selectedProduct = {};
          selectedProduct.product_code = productCode;
          selectedProduct.quantity = quantity;
          selectedProduct.regular_purchase_flg = productRegular;
  ...
  if (setRegularTrue) {
    document.querySelector('#regularPurchaseFlg').value = 1;
    let tr_regularCycle = document.querySelector('#tr_regularCycle');
    let cycleSelectEle = document.querySelector('#regularPurchaseCycle');
    let cycleLabelEle = document.querySelector('#regularPurchaseCycleLabel');
    let cycle = [];

    let cycleStr = '';
    for (let p = 0; p < PRODUCT_CATEGORY_MAPPING.length; p ++) {
      if (PRODUCT_CATEGORY_MAPPING[p].cycle) {
        if (!cycleStr) {
          cycleStr = PRODUCT_CATEGORY_MAPPING[p].cycle.trim();
        } else {
          cycleStr += ',' + PRODUCT_CATEGORY_MAPPING[p].cycle;
        }
      }
    }

    let tempCycleArray = cycleStr.split(',').map(function (item) {
      return item.trim();
    })
    if (!cycle) {
      cycle = tempCycleArray
    } else {
      tempCycleArray.forEach(function (item) {
        if (item && !cycle.includes(item)) {
          cycle.push(item);
        }
      })
    }
    cycle = cycle.sort(function (a, b) {
      return a - b
    })

    if (cycle.length) {
      for (let c = 0; c < cycle.length; c ++) {
        let opt = new Option(cycle[c], cycle[c]);
        cycleSelectEle.append(opt)
      }
      if (cycle.length == 1) {
        cycleLabelEle.innerHTML = cycle[0];
        cycleSelectEle.setAttribute('style', 'display:none;');
      }
      tr_regularCycle.setAttribute('style', 'display:block;');
    } else {
      console.error('有定期商品，但尚未設定配送週期。')
    }
  }
